...
### 2.6 Localization Strategy

다국어는 나중에 문자열만 번역하는 문제가 아니라, 지금부터 구조를 분리해 두어야 하는 항목입니다.

원칙:
- UI 기본 언어는 영어 (`en`)
- 한국어 (`ko`)를 첫 번째 추가 지원 언어로 제공
- 이후 언어 추가를 감안해 locale 비의존적인 stable id를 유지
- prompt 조립의 source language는 영어로 유지
- 사용자 초안은 입력 언어와 상관없이 원문 그대로 보존

분리해야 하는 대상:
1. locale 비의존 `id`
2. 영어 기준 source string
3. locale별 UI label
4. 사용자 입력 원문

MVP 기대 동작:
- UI 라벨은 현재 앱 locale을 따른다
- 조립된 base prompt는 기본적으로 영어로 생성된다
- 한국어 입력은 `User Draft` 섹션에서 원문 그대로 유지된다
- 후속 언어 추가는 `Localizable.xcstrings` 또는 locale별 taxonomy label resource 추가로 확장 가능해야 한다
...
> 상세한 상태 관리, context 유지, 참고 문서 처리, 모델 변경 규칙은 `docs/llm-integration.md`에서 별도 정의합니다.
...
| Locale Resources | `Localizable.xcstrings` 또는 locale별 JSON | 영어 기본 UI + 한국어 지원 + 추후 언어 확장 |
...
### 4.1.1 Project Reconfiguration Plan

현재 실제 Xcode 프로젝트는 기본 템플릿 구조에 가깝고, 이 스펙은 기존 구조에 기능을 덧붙이는 것이 아니라 **앱 구조를 재구성하는 작업**을 전제로 합니다.

Phase 1 시작 전 선행 작업:
- `ContentView.swift`, `Item.swift`, SwiftData 템플릿 의존성 제거
- `floatingboardApp.swift`를 `MenuBarExtra` 또는 `NSStatusItem` + `NSPanel` 중심 구조로 전환
- Feature-First 디렉토리 구조 생성
- `PromptBuilderView`를 새로운 앱 진입 UI로 설정
- SwiftData를 유지할 이유가 없으면 관련 설정과 모델 컨테이너 제거

원칙:
- 템플릿 코드를 살리기 위한 우회보다, 현재 제품 구조에 맞는 재구성을 우선
- 구현 시작 시 "템플릿 유지"와 "제품 구조 이행"을 동시에 하려 하지 않음
...
### 4.4 Dependency Injection Strategy

MVP에서는 과한 DI 프레임워크를 도입하지 않고 **수동 DependencyContainer**를 사용합니다.

원칙:
- `FloatingBoardApp`가 최상위 composition root 역할을 맡는다
- `DependencyContainer`가 Repository와 UseCase의 생성 책임을 가진다
- View는 ViewModel만 알고, ViewModel은 UseCase 프로토콜만 안다
- `@Environment` 또는 명시적 생성자 주입으로 상위에서 하위로 의존성을 전달한다
...
권장 흐름:
`App -> DependencyContainer -> ViewModel -> UseCase -> Repository`
...
### 5.1.1 Floating Panel Lifecycle

Floating Panel은 단순 표시 컴포넌트가 아니라, prompt builder 세션의 시작점입니다.
...
- `Esc`
- 명시적 닫기 버튼
- 패널 외부 클릭 여부는 구현 시 결정하되, 세션 정리 규칙과 일치해야 함

동작 규칙:
- 패널이 열리면 새 builder 세션이 시작된다
- 패널이 닫히면 in-flight LLM 요청은 취소된다
- `Settings` 창이 열려 있어도 Floating Panel은 독립적으로 열고 닫을 수 있어야 한다
- `Settings`는 modal blocker가 아니라 병렬 관리되는 보조 창이다
- MVP에서는 패널 위치 고정 또는 마지막 위치 복원 중 하나를 명시적으로 선택해야 하며, 구현 시작 전 결정이 필요하다
...
- LLM 호출은 별도의 session/state 계약을 따라야 하며, 상세 규칙은 `docs/llm-integration.md`에 위임
...
| 작업 | 산출물 | 완료 기준 |
|------|--------|-----------|
| 템플릿 코드 제거 및 재구성 | 기본 Xcode 템플릿 제거 | 제품 구조와 무관한 SwiftData 템플릿 코드 제거 완료 |
